...
export async function PublishMessage(exchange: string, routingKey: string, content: string, headers?: any) {
    try {
        ...
        const sent = channel.publish(exchange, routingKey, Buffer.from(content), {
            persistent: true,
            headers,
        }, (err, ok) => {
            if (err) {
